...
namespace ThaDasher
{
	...
	private static List<Socket> long_highs = new List<Socket>();
	...
	{   
	    ...
	    var hed = header_template.Replace("%host%", host).Replace("%length%", HTTP.DASHLORIS4.BYTES.Text);
	    
	    var result = long_high.BeginConnect(host, port, null, null);
	    var succes = result.AsyncWaitHandle.WaitOne(500, true);

	    long_highs.Add(long_high);
